//
//  WSelectPhotoPickerAssetsViewController.m
//  WNSelectPhoto
//
...
@interface WSelectPhotoPickerAssetsViewController () <WPhotoPickerCollectionViewDelegate>

// View
@property (nonatomic , strong) WSelectPhotoPickerCollectionView *collectionView;
...
@implementation WSelectPhotoPickerAssetsViewController
...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets{
    ...
    for (WSelectPhotoAssets *assets in selectPickerAssets) {
        if ([assets isKindOfClass:[WSelectPhotoAssets class]]) {
...
- (WSelectPhotoPickerCollectionView *)collectionView{
    if (!_collectionView) {
        ...
        WSelectPhotoPickerCollectionView *collectionView = [[WSelectPhotoPickerCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        // 时间置顶
        collectionView.status = WPickerCollectionViewShowOrderStatusTimeDesc;
        collectionView.translatesAutoresizingMaskIntoConstraints = NO;
        [collectionView registerClass:[WSelectPhotoPickerCollectionViewCell class] forCellWithReuseIdentifier:_cellIdentifier];
        // 底部的View
        [collectionView registerClass:[WSelectPhotoPickerFooterCollectionReusableView class]  forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:_footerIdentifier];
...
- (void) setupAssets{
    ...
    [[WSelectPhotoPickerDatas defaultPicker] getGroupPhotosWithGroup:self.assetsGroup finished:^(NSArray *assets) {
        [assets enumerateObjectsUsingBlock:^(ALAsset *asset, NSUInteger idx, BOOL *stop) {
            WSelectPhotoAssets *wAsset = [[WSelectPhotoAssets alloc] init];
            wAsset.asset = asset;
            [assetsM addObject:wAsset];
        }];
...
- (void)setAssetsGroup:(WSelectPhotoPickerGroup *)assetsGroup{
...
- (void)preview{
    WSelectPhotoBrowserViewController *browserVc = [[WSelectPhotoBrowserViewController alloc] init];
...
- (void) pickerCollectionViewDidSelected:(WSelectPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(WSelectPhotoAssets *)deleteAssets{
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        WSelectPhotoAssets *asset = [pickerCollectionView.lastDataArray lastObject];
        if (deleteAssets){
            asset = deleteAssets;
        }
        ...
        for (NSInteger i = 0; i < self.selectAssets.count; i++) {
            WSelectPhotoAssets *photoAsset = self.selectAssets[i];
