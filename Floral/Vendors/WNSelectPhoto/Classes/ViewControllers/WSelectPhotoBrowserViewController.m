//
//  WSelectPhotoBrowserViewController.m
//  WNSelectPhoto
//
...
#import "WSelectPhotoBrowserViewController.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import "WSelectPhotoPickerBrowserPhotoScrollView.h"
#import "WSelectPhotoCommon.h"
#import "UIImage+WTint.h"
#import "UIView+WExtension.h"

// 分页控制器的高度
static NSInteger WPickerColletionViewPadding = 20;
static NSString *_cellIdentifier = @"collectionViewCell";

@interface WSelectPhotoBrowserViewController () <UIScrollViewDelegate,WPhotoPickerPhotoScrollViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation WSelectPhotoBrowserViewController
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flowLayout.minimumLineSpacing = WPickerColletionViewPadding;
        flowLayout.itemSize = self.view.ml_size;
        flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.ml_width + WPickerColletionViewPadding,self.view.ml_height) collectionViewLayout:flowLayout];
...
- (UILabel *)makeView{
    if (!_makeView) {
        ...
        makeView.backgroundColor = circleColor;
...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        [deleleBtn setImage:[UIImage imageNamed:WSelectPhotoSrcName(@"AssetsPickerChecked")] forState:UIControlStateNormal];
...
- (void)deleteAsset{
    ...
    if ([_deleteAssets valueForKeyPath:currentPage] == nil) {
        ...
        [self.deleleBtn setImage:[[UIImage imageNamed:WSelectPhotoSrcName(@"AssetsPickerChecked") ] imageWithTintColor:[UIColor grayColor]] forState:UIControlStateNormal];
        ...
        [self.deleleBtn setImage:[UIImage imageNamed:WSelectPhotoSrcName(@"AssetsPickerChecked") ] forState:UIControlStateNormal];
    }
...
- (void) reloadData{
    ...
    if (self.currentPage >= 0) {
        ...
        if (self.currentPage == self.photos.count - 1 && self.currentPage > 0) {
            attachVal = WPickerColletionViewPadding;
        }
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        WSelectPhotoAssets *photo = self.photos[indexPath.item]; //[self.dataSource photoBrowser:self photoAtIndex:indexPath.item];
        ...
        WSelectPhotoPickerBrowserPhotoScrollView *scrollView =  [[WSelectPhotoPickerBrowserPhotoScrollView alloc] init];
        ...
        if (self.photos.count == 1) {
            ...
            scrollView.ml_x = -WPickerColletionViewPadding;
        }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ((currentPage < self.photos.count -1) || self.photos.count == 1) {
        ...
        tempF.origin.x = -WPickerColletionViewPadding;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    NSInteger currentPage = (NSInteger)scrollView.contentOffset.x / (scrollView.ml_width - WPickerColletionViewPadding);
